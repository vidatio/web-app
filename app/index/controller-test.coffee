...
xdescribe "Index Controller", ->
    beforeEach ->
        ...
        inject ($controller, $rootScope, $httpBackend, $compile) ->
            ...
            @scope.categories = [{name: "Arbeit"}, {name: "Bildung"}, {name: "Wirtschaft"}, {name: "Sport"},
                {name: "Audi"}]

            $compile("<video id='video'></video>")(@scope)
            $rootScope.$digest()

            IndexCtrl = $controller "IndexCtrl", {$scope: @scope, $rootScope: @rootScope}

            @httpBackend.whenGET('languages/de.json').respond({})
    ...
    describe "on call prepareChartData", ->
        it "the bubble-chart data should be prepared for d3plus -> only the data of categories used in datasets should be prepared", ->
            ...
            expect(chartData).toEqual([{name: "Arbeit", datensätze: 3, color: "#11DDC6"},
                {name: "Bildung", datensätze: 5, color: "#FF5444"},
                {name: "Wirtschaft", datensätze: 4, color: "#000000"},
                {name: "Sport", datensätze: 8, color: "#11DDC6"}])
    ...
    describe "on call setBubblePositions", ->
        it "the positions for the category-bubbles should be calculated and set -> only as much positions as bubbles available should be set", ->
            ...
            expect(finalPositions).toEqual([{name: "Sport", x: 0, y: 0}, {name: "Bildung", x: 28, y: 0},
                {name: "Wirtschaft", x: -14, y: 24}, {name: "Arbeit", x: 14, y: 24}])
            ...
            expect(finalPositions).toEqual([{name: "Audi", x: 0, y: 0}, {name: "Sport", x: -28, y: 0},
                {name: "Bildung", x: 28, y: 0}, {name: "Wirtschaft", x: -14, y: 24}, {name: "Arbeit", x: 14, y: 24}])
